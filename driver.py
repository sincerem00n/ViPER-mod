...
try:
    ray.init(
        num_cpus=min(os.cpu_count(), NUM_META_AGENT) if NUM_META_AGENT > 0 else 1,
        num_gpus=1 if USE_GPU_GLOBAL and torch.cuda.is_available() else 0,
...
def main():
    ...
    try:
        ...
        print(f"Launched {NUM_META_AGENT} meta agents")
    except Exception as e:
        print(f"Failed to launch meta agents: {e}")
        return
    ...
    for i, meta_agent in enumerate(meta_agents):
        try:
            ...
            print(f"Failed to submit job to agent {i}: {e}")
